...
export default function ExpenseForm({ groupId, members = [], initialData = null, onSubmit, onCancel, submitting }) {
  ...
  function resolveInitialPaidBy() {
    if (!initialData) return members[0]?.email || ''
    const match = members.find(
      (m) => m.userId === initialData.paidByUserId || m.id === initialData.paidByUserId
    )
    return match?.email || members[0]?.email || ''
  }

  function resolveInitialSplitBetween() {
    if (!initialData) return members.map((m) => m.email)
    const splitUserIds = (initialData.splits || []).map(
      (s) => s.userId ?? s.participantUserId ?? s.memberId
    )
    if (!splitUserIds.length) return members.map((m) => m.email)
    return members
      .filter((m) => splitUserIds.includes(m.userId) || splitUserIds.includes(m.id))
      .map((m) => m.email)
  }

  const [description, setDescription] = useState(initialData?.description || '')
  const [amount, setAmount] = useState(initialData?.amount ? String(initialData.amount) : '')
  const [paidBy, setPaidBy] = useState(resolveInitialPaidBy())
  const [splitBetween, setSplitBetween] = useState(resolveInitialSplitBetween())
